  ...
  <div class="content">
    <div class="main">
      <div class="left">
        <div class="left-name">
          <div>
            <span class="item">{{formatterTitle(params.topText) || '--'}}</span>各案由出现次数
          </div>
        </div>
        <div class="chart-info" ref="chart">
          <div class="chart-wrap" id="chart">
            <div class="chart" v-for="item in tableData">
              <div class="name">
                <div class="item">{{item[0].timeFrame}}</div>
              </div>
  ...
  methods: {
    ...
    getChartData() {
      ...
      this.request('indicators/getStatisticsData', this.params).then((res) => {
        ...
        let tempArr = {}
        arr.map((i) => {
          if (tempArr[i.timeFrame]) {
            tempArr[i.timeFrame].push(i)
          } else {
            tempArr[i.timeFrame] = [i]
          }
        })
