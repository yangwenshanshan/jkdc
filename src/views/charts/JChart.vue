  ...
  <div class="content">
    <div class="main">
      <div class="left">
        <div class="left-name">
          <div>
            <span class="item">{{display || formatterTitle(params.topText) || '--'}}</span>各类型银行罚单数
          </div>
    ...
    <div class="right">
      ...
      >
        ...
        <el-table-column prop="unit_type" label="银行类型">
          <template slot-scope="scope">{{scope.row.unit_type || '未披露'}}</template>
        </el-table-column>
      ...
      this.requestSaas('/ssssindicators/getStatisticsData', 
      {
        ...
        if (this.tableData.length > 0) {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            xData.push(this.tableData[i].unit_type || '未披露')
            yData.push(this.tableData[i].sum_penalty_number)
          }
          init(this.$refs.chart, xData.reverse(), yData.reverse())
        } else {
