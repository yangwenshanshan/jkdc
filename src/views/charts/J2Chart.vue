  ...
  <div class="content">
    <div class="main">
      <div class="left">
        <div class="left-name">
          <div>
            <span class="item">{{formatterTitle(params.topText) || '--'}}</span>各受罚领域案由出现次数
          </div>
      ...
      <!-- <div class="bottom" v-if="templateText"></div> -->
    </div>
    <div class="right">
      <div class="title">
      ...
      >
        ...
        <el-table-column prop="functional_areas_name1" label="受罚领域"></el-table-column>
        <el-table-column prop="sum_penalty_number" label="案由出现次数"></el-table-column>
  ...
  methods: {
    ...
    getChartData() {
      ...
      this.request('indicators/getStatisticsData', this.params).then((res) => {
        ...
        this.templateText = res.topText
        if (this.tableData.length > 0) {
          let xData = [],
            yData = [[], []]
          for (let i = 0; i < this.tableData.length; i++) {
            xData.push(this.tableData[i].functional_areas_name1)
            yData[0].push(this.tableData[i].sum_penalty_number)
            yData[1].push(this.tableData[i].percent.replace('%', ''))
          }
          xData.reverse()
          yData[0].reverse()
