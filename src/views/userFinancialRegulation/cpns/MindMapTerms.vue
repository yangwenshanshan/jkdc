<template>
  <div class="mind-map-container">
    <div class="tools">
      <el-button size="small" type="primary" @click="handleBatchDownloadTerms">下载法条</el-button>
    ...
    <div class="content" v-if="!loading && !isEmpty">
      <MindMapTree
        ...
        />
      <MindMapOutLine v-if="viewType === 'outline'" :isTerms="true" :treeTags="treeTags" :items="items" :newTree="newTree" />
      <MindMapText v-if="viewType === 'text'" :isTerms="true" :treeTags="treeTags" :items="items" :newTree="newTree" />
    </div>
  ...
  methods: {
    ...
    async updateTag() {
      ...
      this.treeTags = tags.map((v) => {
        const map = JSON.parse(v.mind_map)
        const ksTagNames = new Set();
        ...
        return {
          ...
          map,
          ksTagNames: Array.from(ksTagNames),
          isTerms: true,
        }
      })
    },
    async updateItems() {
      ...
      const [newRes, res] = await Promise.all([this.request(`/flows/trigger/c38de90f-02c0-478c-a630-c9ce04d78021`, newQueryParams, "GET"), this.request(`items/cl_regulation_terms`, this.queryParams, "GET")]);
      ...
      if (res.data && res.data.length) {
        let results = [];
        const groupedRes = _.groupBy(res.data, 'regulation.id');
        _.mapValues(groupedRes, (group) => {
        const item = group[0];
        results.push({
      ...
      //   const k = `${curr.kstag_name}@${curr.regulation.id}`;
      //   if (!acc[k]) {
      //     acc[k] = [];
      //   }
      //   acc[k].push(curr);
      ...
      //   return {
      //     ...firstItem,
      //     name: firstItem.regulation.name,
      //     children: group,
      //   }
      // });
      ...
      //   expandAll: true,
      // });
      // const nodeSize = getTreeSize(tree);
      const nodeSize = getTreeSize(newRes);
      console.log('节点数量 --->', nodeSize);
    ...
    handleBatchDownloadTerms() {
      this.$emit('handleDownload', this.termsKnowledgeTable);
    },
