...
import pdfjsLib from 'pdfjs-dist'; // TODO:异步import()

function getDocument(urlBlob) {
  ...
  console.log(import('pdfjs-dist/webpack'))
  return import(
    /* webpackChunkName: 'pdfjs-dist' */
    'pdfjs-dist/webpack').then(pdfjs => {
      return pdfjs.getDocument({data:urlBlob});
    });
}
      ...
      handler(url) {
        this.request(url, {}, 'GET', 'blob').then(data => {
            ...
            reader.onloadend = (e) => {
              ...
              pdfjsLib.getDocument(rawArray).then(pdf => {
                this.pdf = pdf;
              }).catch(response => {
